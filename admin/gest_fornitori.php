<?php 
	...
	if($search_nome OR $search_cogn) {
		$query = $con->query("SELECT * FROM operatori INNER JOIN distr ON operatori.id_distr = distr.id WHERE nome_oper LIKE '%".$search_nome."%' 
		AND cognome_oper LIKE '%".$search_cogn."%' LIMIT " . $page_first_result . ',' . $results_per_page);
	}else {
